...
namespace jojo
{
	...
	bool jojo::Json::Load(const std::string& filename, rapidjson::Document& document)
	{
		std::string buffer;
		if (!jojo::readFile(filename,buffer))
		{
	...
	bool jojo::Json::Read(const rapidjson::Value& value, const std::string& name, int& data, bool required)
	{
